...
namespace Mangaanya.ViewModels
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        // 統一通知システムイベント
        public event Action<string, string, int>? ShowNotificationEvent; // title, message, autoHideSeconds
        ...
        private async Task IncrementalScanAsync()
        {
            try
            {
                ...
                if (scanResult.Success)
                {
                    ...
                    // 変更がない場合の特別なメッセージ
                    if (scanResult.FilesAdded == 0 && scanResult.FilesUpdated == 0 && scanResult.FilesRemoved == 0)
                    {
                        await ShowDelayedSuccessNotification("差分スキャン完了", $"変更されたファイルはありませんでした。\n処理時間: {duration.TotalSeconds:F1}秒", 0, 300, true);
                    }
                    else
                    {
                        await ShowDelayedSuccessNotification("差分スキャン完了", $"追加: {scanResult.FilesAdded}件\n更新: {scanResult.FilesUpdated}件\n削除: {scanResult.FilesRemoved}件\n処理時間: {duration.TotalSeconds:F1}秒", 0, 300, true);
                    }
                }
                else
                {
                    ...
                    ShowErrorNotificationEvent?.Invoke("差分スキャン完了（エラーあり）", $"追加: {scanResult.FilesAdded}件\n更新: {scanResult.FilesUpdated}件\n削除: {scanResult.FilesRemoved}件\nエラー: {scanResult.Errors.Count}件\n\n最初の3件のエラー:\n{string.Join("\n", scanResult.Errors.Take(3))}" + 
                        (scanResult.Errors.Count > 3 ? $"\n...他{scanResult.Errors.Count - 3}件のエラー" : ""), 6);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task CleanupOrphanedThumbnailsAsync()
        {
            try
            {
                ...
                // 確認ダイアログを表示
                var result = await ShowConfirmationAsync("サムネイル掃除確認", "データベースにひも付かない孤立したサムネイルファイルを削除します。\n\n実行しますか？");
                if (!result)
                {
                ...
                if (cleanupResult.Success)
                {
                    ...
                    {
                        await ShowDelayedSuccessNotification("サムネイル掃除完了", $"削除されたファイル: {cleanupResult.DeletedCount}件", 0, 300, true);
                    }
                    else
                    {
                        // 削除対象がない場合も遅延表示を適用
                        await ShowDelayedSuccessNotification("サムネイル掃除完了", "削除対象の孤立ファイルはありませんでした。", 0, 300, true);
                    }
                }
                else
                {
                    ...
                    ShowErrorNotificationEvent?.Invoke("サムネイル掃除エラー", $"サムネイル掃除中にエラーが発生しました: {cleanupResult.ErrorMessage}", 4);
                }
            }
            catch (Exception ex)
            {
                ...
                ShowErrorNotificationEvent?.Invoke("サムネイル掃除エラー", $"サムネイル掃除中にエラーが発生しました: {ex.Message}", 4);
            }
            finally
            {
                IsProcessing = false;
        ...
        private async Task DiagnoseThumbnailStatusAsync()
        {
            try
            {
                ...
                // デバッグ用：診断結果の長さをログに出力
                _logger.LogInformation("サムネイル診断結果の長さ: {Length}文字", diagnosticResult?.Length ?? 0);
                
                ShowNotificationEvent?.Invoke("サムネイル診断結果", diagnosticResult ?? "診断結果が取得できませんでした", 0); // 0 = 自動非表示なし、手動でのみ閉じる
            ...
            catch (Exception ex)
            {
                ...
                ShowErrorNotificationEvent?.Invoke("サムネイル診断エラー", $"サムネイル診断中にエラーが発生しました: {ex.Message}", 4);
            }
            finally
            {
                IsProcessing = false;
        ...
        private async Task MoveFilesAsync()
        {
            try
            {
                ...
                // フォルダ選択ダイアログを表示
                var selectedFolder = await ShowFolderSelectionDialogAsync();
                if (string.IsNullOrEmpty(selectedFolder))
                {
                    ...
                    // キャンセル時の結果を表示
                    var cancelledResult = new FileMoveResult { IsCancelled = true };
                    await ShowMoveResultAsync(cancelledResult);
                    return;
                }
        ...
        /// フォルダ選択ダイアログを表示します
        /// </summary>
        private async Task<string?> ShowFolderSelectionDialogAsync()
        {
            try
            {
