    ...
    public class FileMoveService : IFileMoveService
    {
            ...
            FileMoveProgress progressInfo)
        {
            ...
            try
            {
                ...
                // 移動先の既存ファイル情報を取得（上書きされる場合のサムネイル削除用）
                ...
                if (File.Exists(destinationFilePath))
                {
                    try
                    {
                        ...
                        {
                            _logger.LogInformation("移動先の既存ファイルが上書きされます: {FilePath} (ID: {Id}, ThumbnailPath: {ThumbnailPath})", 
                                destinationFilePath, overwrittenFile.Id, overwrittenFile.ThumbnailPath ?? "なし");
                        }
                        else
                        {
                ...
                // 移動先の既存ファイルのサムネイルファイルを削除
                if (overwrittenFile != null && !string.IsNullOrEmpty(overwrittenFile.ThumbnailPath))
                {
                    try
                    {
                        var thumbnailDeleted = await _thumbnailService.DeleteThumbnailAsync(overwrittenFile.ThumbnailPath);
                        if (thumbnailDeleted)
                        {
                            _logger.LogDebug("上書きされたファイルのサムネイルを削除しました: {ThumbnailPath}", overwrittenFile.ThumbnailPath);
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "上書きされたファイルのサムネイル削除に失敗しました: {ThumbnailPath}", overwrittenFile.ThumbnailPath);
                    }
                }
            ...
            IProgress<FileMoveProgress>? progress)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("データベース同期処理が完了しました: {Count}件", successfulMoves.Count);
            }
            catch (Exception ex)
            {
